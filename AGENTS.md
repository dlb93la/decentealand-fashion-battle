...
## Project-specific authoring choice
The user explicitly requested a code-only workflow. Create scene entities in TypeScript; Creator Hub and composite authoring are not required. This is the standalone Fashion Battle game. Preserve the existing game loop and keep changes focused on the requested polish and fixes.

## Standalone project
This folder is the repository root. Do not depend on files from a parent Decentraland or experiences folder.
Bundled SDK skills live in `.agents/skills/`; start with `.agents/skills/sdk-scenes/SKILL.md`.
Read `README.md` and `docs/PROJECT-STATUS.md` for current status. Historical handoffs and QA reports are dated evidence, not current instructions.
Use `npm run start:mobile` for the mobile preview on port 8010. World destination: `leined.eth`; wallet signing is handled by the owner.
`backups/`, SDK skills, reports and editor settings are development files and must not ship with the scene.
